...
test_that("hmrf works", {
  ...
  res <- hmrf(pval, adj, seedindex, pthres = 0.5, iter = 2)

  expect_true(is.list(res))
  expect_true(all(c("Iupdate", "post", "b", "c", "mu1", "sigmas") %in% names(res)))
})
...
test_that("hmrf can get c to be large", {
  ...
  res <- hmrf(pval, adj, seedindex, pthres = 0.05, iter = 10)

  #do another case where pval is uninformative
  pval2 <- c(0.01, stats::runif(d-2), 0.01)
  res2 <- hmrf(pval2, adj, seedindex, pthres = 0.05, iter = 10)
