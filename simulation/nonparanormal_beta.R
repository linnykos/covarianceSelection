...
criterion <- function(dat, vec, y){
  ...
  error_our <- abs(mgcv::slanczos(cov_our - cov_oracle, k = 1)$values[1])
  error_base <- abs(mgcv::slanczos(cov_base - cov_oracle, k = 1)$values[1])
  error_all <- abs(mgcv::slanczos(cov_all - cov_oracle, k = 1)$values[1])
  
  list(partition_idx = obj$null_idx, idx_our = idx_our, error_our = error_our,
       error_base = error_base, error_all = error_all)
}
...
res <- simulation::simulation_generator(rule = rule, criterion = criterion,
                                        ...
                                        cores = NA, as_list = T,
                                        filepath = "../results/nonparanormal_beta_tmp.RData",
                                        verbose = T)
save.image("../results/nonparanormal_beta.RData")