rm(list=ls())
load("../results/clique_simulation_small_tmp.RData")

paramMat <- cbind(15, 5, 5, 500, 100, c(0, 0.05, 0.1, 0.25))
colnames(paramMat) <- c("group1", "group2", "group3", "n", "d", "kappa")

# remove NAs in res
res <- res[1:2]
for(i in 1:2){
  res[[i]] <- res[[i]][which(sapply(res[[i]], length) > 1)]
}
...
png("../figures/individual_lvl1_tmp.png", height = 800, width = 2000, res = 300, units = "px")
...
png("../figures/individual_lvl2_tmp.png", height = 800, width = 2000, res = 300, units = "px")
trials <- ncol(hyp_fpr_list[[lvl]][[1]])
lvl <- 2
name_vec <- c("Our method", "Naive method (Bonferroni)", "Naive method (BH)")
par(mfrow = c(1,length(name_vec)))
for(j in 1:length(name_vec)){
...
png("../figures/partition_lvl1_tmp.png", height = 800, width = 2000, res = 300, units = "px")
...
png("../figures/partition_lvl2_tmp.png", height = 800, width = 2000, res = 300, units = "px")
trials <- ncol(partition_fpr_list[[2]][[1]])
lvl <- 2
name_vec <- c("Our method", "Naive method (Bonferroni)", "Naive method (BH)")
par(mfrow = c(1,length(name_vec)))
for(j in 1:length(name_vec)){
